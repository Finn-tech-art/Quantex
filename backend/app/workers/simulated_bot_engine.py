...
def _start_new_session(bot: dict, sim: dict) -> None:
    # No free-tier budget check here on purpose — that's enforced once, at
    # creation/configuration time, in simulated_bot_service.
    # create_simulated_bot. Since MAX_SESSIONS is 1, this only ever runs
    # once per bot's lifetime anyway (the bot caps right after, see
    # _advance_pending_session below), so there's nothing left to gate here.
